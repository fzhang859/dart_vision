...
constexpr std::uint16_t kCRC16Init = 0xffff;
...
std::uint16_t calculateCRC16(const std::uint8_t* data, std::size_t length) {
    ...
    std::uint16_t crc16 = kCRC16Init;

    while (length--) {
        crc16 = (crc16 >> 8) ^ kCRC16Table[(crc16 ^ (*data++)) & 0x00ff];
    }
...
bool verifyCRC16(const std::uint8_t* data, std::size_t length) {
    ...
    std::uint16_t expected = calculateCRC16(data, length - 2);
    bool result =
        ((expected & 0x00ff) == data[length - 2]) && ((expected >> 8) == data[length - 1]);
    return result;
}
...
void appendCRC16(std::uint8_t* data, std::size_t length) {
    ...
    uint16_t crc16 = calculateCRC16(data, length - 2);
    data[length - 2] = (crc16 & 0x00ff);
